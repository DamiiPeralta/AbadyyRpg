...
public class WorldMapActionToastUI : MonoBehaviour
{
    ...
    public static WorldMapActionToastUI GetOrCreate()
    {
        WorldMapActionToastUI existing = FindObjectOfType<WorldMapActionToastUI>();

        if (existing != null)
            return existing;

        GameObject go = new GameObject("WorldMapActionToastUI");
        return go.AddComponent<WorldMapActionToastUI>();
    }

    public void ShowSuccess(string body)
    {
        Show("Recursos encontrados", body);
    }

    public void ShowFailure(string body)
    {
        Show("No se pudo buscar", body);
    }
    ...
    private void EnsureBuilt()
    {
        ...
        titleText = CreateText("Title", panelRoot.transform, "", 22, TextAlignmentOptions.Center);
        bodyText = CreateText("Body", panelRoot.transform, "", 18, TextAlignmentOptions.Center);
    }

    private GameObject CreatePanel(Transform parent)
    {
        GameObject panel = new GameObject("Panel_WorldMapActionToast");
        panel.transform.SetParent(parent, false);

        Image image = panel.AddComponent<Image>();
        image.color = new Color(0.07f, 0.08f, 0.06f, 0.93f);
